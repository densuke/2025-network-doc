...
なお、ここでは暗号に関して、共通鍵と公開鍵をもちいています。これらの概要については、付録に記載のものを参考にしてください。
...
3. **サーバー証明書**: これは、サーバーによって発行される証明書です。サーバー証明書は、サーバーの公開鍵とサーバーの情報を含みます。
...
```{note}
実際佐藤もそういうところから証明書にサインをしてもらっていましたが、当時では年額3000円ほどで、別に高いと言うほどでもありませんでした。
...
現在では大半のルート証明書として、Let's Encryptが発行した証明書が含まれています。
