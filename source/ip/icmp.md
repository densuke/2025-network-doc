...
- 2オクテットの『チェックサム』
- 2オクテットの『識別子』(データ)
    - データ部分の長さや、種類に応じたデータや元パケットの情報などが適宜渡されます
        - 例えばEchoメッセージ(タイプ8や0)では、識別子(2オクテット)とシーケンス番号(2オクテット)となります
        - その他メッセージの種類に応じた情報や、エラー通知においては元のIPパケットのヘッダの一部などが格納されています
        - この部分にどのようなデータが入るべきかは[RFC792（日本語訳）](https://pentan.info/doc/rfc/j792.html)に記載されています。
...
TTLを消費しきったときに発生するもので

- 前述した`ping`や`traceroute`のように、TTLを操作するとき
- ルーティング上のミスでループをしてしまっているようなパケット

といったもので発生することになります。タイプは11で、データ部分には元のIPパケットのヘッダ情報が含まれます。
