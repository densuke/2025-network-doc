...
3.  **NATテーブルへの登録**: この変換ルールは事前に設定されているため、動的なエントリの追加は行われません。NATデバイスは、このルールを基に通信を処理します。ただし、戻りパケットのためにNATテーブルのエントリを追加しておく必要はあるため、以下のような形でNATテーブルにエントリが追加されます。

    NATテーブルへの追加登録例:

    | プロトコル | 内部IPアドレス | 内部ポート | 外部IPアドレス | 外部ポート | TTL |
    | :--------: | :------------: | :--------: | :------------: | :--------: | :-: |
    |    TCP     |  192.168.1.10  |     80     |  203.0.113.1   |     80     |  *  |
    |    TCP     |  203.0.113.200 |   40000    |  203.0.113.1   |     80     |  300  |
