...
スイッチは、MACアドレスを利用してネットワークを中継する装置であり、データリンク層を理解する上で重要な機器です。
...
イーサネットヘッダから宛先MACアドレスを抽出し、転送先のポートを決定できると説明しましたが、スイッチはどのようにしてMACアドレスとポートの対応を把握しているのでしょうか。

- スイッチには内蔵メモリに、過去に受信したフレームの情報から『どのポートにどのMACアドレスがあるのか』を学習し記憶できます、この対応表(MACアドレステーブル)を辞書のように参照して転送先を決定します
- MACアドレステーブル
    - 前述の『どのポートにどのMACアドレスがあるのか』を学習し記憶したもの
...
MACアドレステーブルの例としては以下のようなものを考えてください
