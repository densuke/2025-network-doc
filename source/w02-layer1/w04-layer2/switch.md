...
イーサネットヘッダから宛先MACアドレスを抽出します。スイッチは、その情報をもとに転送先ポートを決定します。しかし、スイッチはMACアドレスとポートの対応関係をどのように学習し、保持しているのでしょうか。
...
- メモリの及ぼす影響
    ...
    - 未解析のフレームのバッファリングがしやすくなる
        - これにより、一時的なトラフィックの集中に対応できます。
        - 一時的なトラフィック集中の例として、[マイクロバースト](https://www.toyo.co.jp/ict/casestudy/detail/id=42422)と呼ばれる現象があります。
...
![VLANのイメージ](images/vlan.drawio.png)
