...
その際に助けになりそうなものとしては、Ansible Vaultがあります。Ansible Vaultは、Ansibleで使用する変数やファイルを暗号化して保存するための機能です。
...
ここでは、`mysql_user`モジュールを使用して、rootユーザーのパスワードを設定しています。
...
`stat`というアクションが登場しました、`path`で指定したファイルの存在や状態を保持するためのアクションです。処理結果が存在するため、それを保存するために`register`を使用しています。
...
`when`条件は、このタスクた実行されるかを判定するために使います。
