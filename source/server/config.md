...
```{note}
配布しているリポジトリを使う場合はすでに作成済みです(`inventory/hosts.yml`)。
この作業は省略できます。
```
...
```{note}
配布しているリポジトリを使う場合はすでに作成済みです(`ansible.cfg`)。
この作業は省略できます。
```
...
```{note}
配布しているリポジトリを使っている場合、`ansible.cfg`にてインベントリファイルを指定済みのため、`-i hosts.yml`は不要です。
```
...
Factsの内容を確認するには、`setup`モジュールを使います。

```{code-block} bash
$ uv run ansible -i hosts.yml -m setup all | less
#                ^^^^^^^^^^^^配布リポジトリを使っている場合は不要

```

長いので`less`コマンドに渡してゆっくり読むようにしています。様々な情報が取得できていることがわかると思います。
確認できたら、{kbd}`q`キーで`less`から抜けておいてください。

## パッケージ操作

パッケージを導入するモジュールである`apt`を使って、`htop`パッケージをインストールしてみましょう。
